...
library(readxl)
havaK <- readxl::read_xlsx("C:\\Users\\begum\\bilgiyonetim\\Veri Detaylari18.04.2024 01_37_50.xlsx", skip = 1)
print(havaK)
```
...
havaK <- data.frame(
  ...
  Parametre1 = rnorm(100, mean = 20, sd = 5),
  Parametre2 = rnorm(100, mean = 50, sd = 10)
)

# Veri setini dygraph ile çizme
dygraph(havaK, main = "İki Değişkenli Grafik") %>%
  dyAxis("y", label = "Parametre 1", valueRange = c(0, 40)) %>%
  dyAxis("y2", label = "Parametre 2", valueRange = c(0, 100)) %>%
  dySeries("Parametre1", label = "Parametre 1", axis = "y") %>%
  # dySeries("Parametre2", label = "Parametre 2", axis = "y2") %>%
