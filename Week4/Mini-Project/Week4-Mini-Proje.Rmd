...
true_colnames <- dput(colnames(havaK), file = ".colnames")
...
str(havaK_filled_mean)

havaK_filled_mean <- havaK
havaK_filled_mean[,-1] <- sapply(havaK[,-1], fill_na_with_mean)

# Zaman sütunundaki NA değerlerini temizle
cleaned_time <- havaK_filled_mean$Time[complete.cases(havaK_filled_mean$Time)]
# Temizlenmiş zaman etiketleriyle veri çerçevesini alt kümeleyin
cleaned_data <- havaK_filled_mean[complete.cases(havaK_filled_mean$Time), c("Time", "NO ( µg/m3 )", "NO2 ( µg/m3 )")]
# xts nesnesini oluştur
havaK_xts_mean <- xts(cleaned_data[, c("NO ( µg/m3 )", "NO2 ( µg/m3 )")], order.by = cleaned_time)
...
# 'Time' sütunundaki NA değerlerini temizleyin
cleaned_data <- na.omit(havaK_filled_inter[, c("Time", "NO ( µg/m3 )", "NO2 ( µg/m3 )")])

# xts nesnesini oluştur
havaK_xts_inter <- xts(cleaned_data[, c("NO ( µg/m3 )", "NO2 ( µg/m3 )")], order.by = cleaned_data$Time)
