...
seciliSutunlar_mean <- havaK_filled_mean[, c("Time", "NO ( µg/m3 )", "NO2 ( µg/m3 )")]
graph <- dygraph(seciliSutunlar_mean, main = "Hava Kalitesi (Mean)") %>%
...
seciliSutunlar_inter <- havaK_filled_inter[, c("Time", "NO ( µg/m3 )", "NO2 ( µg/m3 )")]

graph <- dygraph(seciliSutunlar_inter, main = "Hava Kalitesi (Interpolation)") %>%
  dyOptions(colors = c("blue", "red")) %>%
  dyRangeSelector()
